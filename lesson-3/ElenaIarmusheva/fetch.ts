...
class Flickr {
    ...
    protected render(body: flickrPhotos ): void {
    ...
    protected search(cb: (body: flickrPhotos) => flickrPhotos): void {
        if (!this.input.value) {
            return;
        }
    ...
    protected getPhotos(input: string|IRequest, cb: (body: flickrPhotos) => flickrPhotos): void {
        fetch(input).then((res: IResponse): Promise<any> => res.json())
            .then(cb);
    }
}
